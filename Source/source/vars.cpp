...
ALLEGRO_BITMAP* bmp_display = NULL;
vector<mob*> marbles;
...
string cutscene_id;
sample_struct msx_alert;
sample_struct msx_today;
sample_struct msx_mission;
...
point cam_box[MAX_PLAYERS][2];
point cam_final_pos[MAX_PLAYERS];
float cam_final_zoom = 1.0;
point cam_pos[MAX_PLAYERS];
float cam_zoom = 1.0;
...
onion* close_to_onion_to_open[MAX_PLAYERS];
pikmin* close_to_pikmin_to_pluck[MAX_PLAYERS];
ship* close_to_ship_to_heal[MAX_PLAYERS];
interactable* close_to_interactable_to_use[MAX_PLAYERS];
mob* closest_group_member[MAX_PLAYERS];
bool closest_group_member_distant[MAX_PLAYERS];
...
size_t cur_leader_nrs[MAX_PLAYERS];
leader* cur_leader_ptrs[MAX_PLAYERS];
string cur_message;
...
vector<float> group_move_arrows[MAX_PLAYERS];
float group_move_magnitude[MAX_PLAYERS] = { 0,0,0,0 };
timer group_move_next_arrow_timer(GROUP_MOVE_ARROWS_INTERVAL);
bool group_move_cursor[MAX_PLAYERS] = { false ,false,false,false};
float group_move_task_range = 0;
movement_struct group_movement[MAX_PLAYERS];
...
mob* leader_cursor_mobs[MAX_PLAYERS];
point leader_cursor_ss[MAX_PLAYERS];
sector* leader_cursor_sectors[MAX_PLAYERS];
point leader_cursor_ws[MAX_PLAYERS];
vector<leader_type*> leader_order;
vector<string> leader_order_strings;
movement_struct leader_movement[MAX_PLAYERS];
...
vector<mob*> mobs;
point mouse_cursor_s[MAX_PLAYERS];
point mouse_cursor_w[MAX_PLAYERS];
...
int playerpts[MAX_PLAYERS] = {0,0,0,0};
...
int scr_h = DEF_SCR_H;
int scr_w = DEF_SCR_W;
ALLEGRO_TRANSFORM screen_to_world_transform[MAX_PLAYERS];
...
bool VERSUS_ON = false;
ALLEGRO_VOICE* voice = NULL;
map<string, weather> weather_conditions;
size_t whistle_control_id[MAX_PLAYERS] = { INVALID , INVALID , INVALID , INVALID };
float whistle_dot_radius[MAX_PLAYERS][6] = { -1, -1, -1, -1, -1, -1 };
float whistle_fade_radiuse = 0;
timer whistle_fade_timere(WHISTLE_FADE_TIME);
float whistle_growth_speed = 180.0f;
size_t pnum;
timer whistle_next_dot_timer[MAX_PLAYERS] = { timer(WHISTLE_DOT_INTERVAL),timer(WHISTLE_DOT_INTERVAL),timer(WHISTLE_DOT_INTERVAL),timer(WHISTLE_DOT_INTERVAL) };
timer whistle_next_ring_timer[MAX_PLAYERS] = { timer(WHISTLE_RINGS_INTERVAL) ,timer(WHISTLE_RINGS_INTERVAL) ,timer(WHISTLE_RINGS_INTERVAL) ,timer(WHISTLE_RINGS_INTERVAL) };
float whistle_radius[MAX_PLAYERS] = { 0,0,0,0 };
vector<unsigned char> whistle_ring_colors[MAX_PLAYERS];
unsigned char whistle_ring_prev_color[MAX_PLAYERS] = { 0,0,0,0 };
vector<float> whistle_rings[MAX_PLAYERS];
bool whistling = false;
bool window_position_hack = false;
ALLEGRO_TRANSFORM world_to_screen_transform[MAX_PLAYERS];
float zoom_max_level = 3.0f;
float zoom_mid_level = 1.4f;
float zoom_min_level = 0.333f;
