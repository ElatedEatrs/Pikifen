    ...
    if(state == EDITOR_STATE_SPRITE_BITMAP) {
        ...
        if(s && s->parent_bmp) {
            ...
            al_transform_coordinates(
                &screen_to_world_transform[pnum], &scene_tl.x, &scene_tl.y
            );
            al_transform_coordinates(
                &screen_to_world_transform[pnum], &scene_br.x, &scene_br.y
            );
...
void animation_editor::draw_side_view_hitbox(
    ...
    if(h_ptr->height == 0) {
        ...
        z_to_use = scr_h + 1;
        h_to_use = 0 - 1;
        al_transform_coordinates(&screen_to_world_transform[pnum], &dummy, &z_to_use);
        al_transform_coordinates(&screen_to_world_transform[pnum], &dummy, &h_to_use);
