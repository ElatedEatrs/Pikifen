...
void area_editor::load() {
   ...
   frm_area->easy_add(
        "lbl_day",
        new lafi::label("Day Picker:"), 50, 48
    );
    frm_area->easy_row();
    frm_area->easy_add(
        "but_day",
        new lafi::button("SunIcon"), 50,48
    );
    frm_area->easy_row();
    frm_area->easy_add(
        "but_paths",
	...
	frm_area->widgets["but_day"]->left_mouse_click_handler =
    [this](lafi::widget*, int, int) {
		open_picker(PICKER_LOAD_DAY);
	};
 ...
 frm_asb->widgets["but_sector_type"]->description =
	...
	frm_mobs->easy_add(
		"but_saveday",
		new lafi::button("","", editor_icons[ICON_SAVE]), 20, 32);              
    y = frm_mobs->easy_row();
	...
	frm_mobs->widgets["but_saveday"]->left_mouse_click_handler =
		[this](lafi::widget*, int, int) {
		area_editor::save_day(false);
	};
	frm_mobs->widgets["but_saveday"]->description =
		"Save the current mob layout as a day.";
        ...
        if(show_path_preview) {
            if(path_preview_checkpoints[0].x == LARGE_FLOAT) {
                ...
                path_preview_checkpoints[0].x =
                    cam_pos[pnum].x - DEF_AREA_EDITOR_GRID_INTERVAL;
    ...
    [this] (lafi::widget*, int, int) {
        ...
        area_editor_quick_play_cam_pos = cam_pos[pnum];
    ...
    cam_zoom = 1.0;
    cam_pos[pnum].x = cam_pos[pnum].y = 0.0;
    ...
    if(!area_editor_quick_play_area.empty()) {
        ...
        area_editor_quick_play_area.clear();
        load_area(false);
        cam_pos[pnum] = area_editor_quick_play_cam_pos;
